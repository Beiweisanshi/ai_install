...
function ConfigPanel({ tools, onSave, onSkip }: ConfigPanelProps) {
  ...
  const [form, setForm] = useState<Record<string, ConfigFormState>>(() =>
    Object.fromEntries(configurableTools.map((tool) => [tool, { api_url: "", api_key: "" }])),
  );
  ...
  const handleSave = () => {
    ...
    const entries: ConfigEntry[] = configurableTools.map((tool) => {
      ...
      const apiUrlError = validateUrl(toolForm.api_url);
    ...
    <section className="flex h-full flex-col gap-4">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pr-1">
        {configurableTools.length === 0 ? (
          ...
          >
            <p className="text-sm">无需配置的工具，可以直接跳过。</p>
          </div>
        ) : (
...
function InputField({
  ...
  return (
    <div>
      <label className="mb-1.5 block text-xs font-medium" style={{ color: theme.textSecondary }}>
        {label}
      </label>
      <input
